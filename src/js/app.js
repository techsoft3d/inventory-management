  ...
  constructor() {
    ...
    this._viewer.setCallbacks({
      modelStructureReady: () => {
        ...
        this._viewer.view
          .getNavCube()
          .setAnchor(Communicator.OverlayAnchor.LowerRightCorner);
      ...
      selectionArray: selectionEvents => {
        ...
        document.getElementById("node-id").innerHTML =
          nodeId.toString() || "Unknown";
        document.getElementById("node-name").innerHTML =
          this._viewer.model.getNodeName(nodeId) || "Node Name Not Defined";
        ...
        if (this._modelData.has(nodeId)) {
          ...
          document.getElementById(
            "inv-select-cost"
          ).innerHTML = `$ ${nodeData.Price.toFixed(2)}`;
          document.getElementById(
            "inv-total-stock"
          ).innerHTML = nodeData.Stock.toString();
          document.getElementById("inv-avail-stock").innerHTML = (
            nodeData.Stock - nodeData.Reserved
          ).toString();
          document.getElementById(
            "inv-claimed"
          ).innerHTML = nodeData.Reserved.toString();
    ...
    this._viewer.model.clear().then(() => {
      ...
      this._viewer.model.loadSubtreeFromScsFile(modelNodeId, directoryPath + "/data/" + modelName + ".scs")
        .then(() => {
          ...
          fetch(directoryPath + "/data/database/" + modelName + ".json").then(
            resp => {
              if (resp.ok) {
                resp.json().then(data => {
          ...
          const modelFileName = this._viewer.model.getModelFileNameFromNode(
            modelRoot
          );
          const modelFileFormat = this._viewer.model.getModelFileTypeFromNode(
            modelRoot
          );
          document.getElementById("model-file-name").innerHTML =
            modelFileName || "N/A";
          document.getElementById("model-file-type").innerHTML =
            Communicator.FileType[modelFileFormat] || "N/A";
        });
    });
  }
