...
export default class DisplayFilter {
    _priceColorMap: Map<number, Communicator.Color>;
    _stockColorMap: Map<number, Communicator.Color>;
    _filteredNodes: number[];
    _viewer: Communicator.WebViewer;
    _sliders: Map<string, HTMLInputElement>;
    _sliderVals: Map<string, string>;
    _companyFilter: string[];
    _filterSelection: string;
    _gradientSelection: string;
    _defaultColors: Map<number, Communicator.Color>;
    ...
    constructor(viewer) {
        ...
        for (let element of selectedCompanies) {
            let inputElement = element as HTMLImageElement;
        ...
        sliderElements.forEach((x) => {
            let slider = x as HTMLInputElement;
            switch (slider.id) {
                case "psMinSlider":
    ...
    updateSliderRange(id: string) : void {
        let sliderElement = document.getElementById(`${id}Slider`) as HTMLInputElement;
        let key  = id + "Val";
        this._sliderVals.set(key, sliderElement.value);
    }


    updateSliderLabels(filter: string) : void {
    ...
    gatherFilteredNodes(modelData: Map<number, nodeDataObj>) : void {
    ...
    captureNativeColors(modelData: Map<number, nodeDataObj>) : void {
        let valuesIterator = modelData.values();
        for (let nodeValues of valuesIterator) {
    ...
    updateColorGradients(modelData: Map<number, nodeDataObj>) : void {
    ...
    setFilterSelection(filter) : void {
        this._filterSelection = filter;
    }

    setGradientSelection(choice) : void {
